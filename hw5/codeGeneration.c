...
int ARoffset = 4;
int t_reg_num = 0;
int t_reg_status[7]; // 0: unused; 1: used, not dirty; 2: used, dirty
FILE *fp;
...
void free_t_reg(int reg_num){
    t_reg_status[reg_num] = 0;
}
                ...
                case WITH_INIT_ID:
                    ...
                    if(symbolTable.currentLevel == 0){
                        if(exprNode->dataType == INT_TYPE){
                            fprintf(fp, "_%s:\t.word %d", idName, exprNode->semantic_value.const1->const_u.intval);
                        }
                        else{
                            fprintf(fp, "_%s:\t.word %d", idName, exprNode->semantic_value.const1->const_u.fval);
                        }
                    }
                    // local declaration
                    else{
